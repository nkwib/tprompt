...
- **`{var}`** — rejected as the default. Real subcommunity (LangChain/BAML port users, Python-influenced teams) but the JSON-collision footgun is unacceptable for the un-configured path. Shipped as a named export (`tprompt/single-brace` or `tprompt/fstring` — exact module name decided at code time) so opt-in is one import away.
...
- The `package.json#homepage` TS Playground link, the `npx tprompt init` scaffold, the README first-paragraph example, and the Vercel AI SDK before/after demo all use `{{var}}`. Changing the default later would require updating all of those surfaces in lock-step and would break inbound links to the Playground.

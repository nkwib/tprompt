...
- [x] Quick-start: a working snippet (`pnpm add tprompt` + the smallest possible example with `.with({...})`; `zod` mentioned as optional)
- [x] Pluggable delimiter section: shows `makePromptTag({ open, close })` AND `import { prompt } from 'tprompt/single-brace'`
